package com.lothus.sync.stats.menus.games.skywars.ability.team;
...
import com.lothus.sync.stats.menus.games.skywars.ability.team.confirm.ConfirmMenu;
...
public class AbilityTeamMenu extends AbstractMenu {


    public AbilityTeamMenu() {
        super(
                "Habilidades - Sky Wars Duplas",
                (9*6)
        );
    }
    ...
    public void items(Player player) {
        ...
        for (Ability kit : Platform.getSkyPlatform().getAbilityController().getAbility()) {
            ...
            inventory.setItem(slot, kit.icon(player, RoomType.DUPLAS));
        }
    }
